...
function StatRow({ label, value, sub, colorClass }: StatRowProps) {
  return (
    <div className="flex items-center justify-between py-2.5 border-b border-slate-100 dark:border-slate-700/60 last:border-0">
      ...
      <div className="text-right">
        <span className={`font-bold text-base leading-none ${colorClass ?? 'text-slate-900 dark:text-white'}`}>
          {value}
        </span>
        {sub && <span className="text-slate-400 dark:text-slate-500 text-xs ml-1">{sub}</span>}
      </div>
    </div>
  );
}
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-slate-900 dark:text-white font-semibold text-sm">Resumen</h2>
          <span className="ml-auto text-slate-400 dark:text-slate-500 text-xs">{dateLabel}</span>
        </div>
      </CardHeader>

      <CardBody className="py-1">
        <StatRow label="Total reservas" value={summary.total} colorClass="text-amber-500 dark:text-amber-400" />
        ...
        <StatRow
          ...
          colorClass={summary.pending > 0 ? 'text-amber-600 dark:text-amber-400' : undefined}
        ...
        <div className="px-5 pb-4">
          ...
          <div className="flex gap-3 mt-2 flex-wrap">
            <span className="flex items-center gap-1 text-xs text-slate-400 dark:text-slate-500">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 inline-block" /> Confirmadas
            </span>
            <span className="flex items-center gap-1 text-xs text-slate-400 dark:text-slate-500">
              <span className="w-1.5 h-1.5 rounded-full bg-amber-500 inline-block" /> Pendientes
            </span>
          </div>
        </div>
      )}
